...
#[pyfunction]
fn print_top_5(pydf: PyDataFrame) -> PyResult<()> {
    // Access the inner DataFrame using .0
    let df = &pydf.0;

    println!("Top 5 rows from Rust (Zero-copy):");
...
#[pyfunction]
fn calculate_rolling_returns_df(pydf: PyDataFrame, column: &str, k: usize) -> PyResult<PyDataFrame> {
    let df = &pydf.0;
    
    // Extract the price column
    let prices = df.column(column)
        .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(
            format!("Column '{}' not found: {}", column, e)
        ))?
        .f64()
        .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(
            format!("Column must be numeric: {}", e)
        ))?;
    
    if prices.len() < k {
        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(
            format!("Not enough data points. Need at least {} prices, got {}", k, prices.len())
        ));
    }
    ...
    let result = DataFrame::new(vec![
        Series::new(series_name.into(), &returns).into(),
    ])
...
fn rollingwins(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(print_top_5, m)?)?;
